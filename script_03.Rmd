...
sep_joined <- 
  ...
  rename(title = title.x, department = department.x, name = name.x,
         u_grad_old = u_grad_new.x, u_grad_new = u_grad_new.y) %>% 
...
sep_joined_sliced %>% 
  ggplot(aes(x = reorder(title, u_grad_diff_abs), y = u_grad_diff_abs,
    ...
    ggtitle("Classes with Five Biggest Increases and Five 
            \n Biggest Decreases in Enrollment, Sep 1st - Sep 21st") +
    theme_minimal() 
