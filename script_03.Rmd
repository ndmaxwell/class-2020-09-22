...
- Specifically decide what we want to know. Drop between Sep 1 and today

- Decide what data is actually relevant - we should be thoughtful about this.
-- This means that we should decide what columns we want (undergrad enrollment is most important)


After we have a model, Temperance suggests we should:
- Recognize the limits of our model. It won't tell us everything and there is unaccounted for variance
- From this model, we can't know why people are dropping - can explain what's happening but not knowing necessary what's happening
- We also can't separate with this dataset who dropped the course and who added the course
...
sep_joined <- 
  inner_join(sep_old, sep_new, by = "id") %>% 
  rename(u_grad_old = u_grad_new.x) %>% 
  rename(u_grad_new = u_grad_new.y) %>% 
  ...
  arrange(u_grad_diff) %>% 
  slice(1:5, 392:396) %>% 
  mutate(positive = ifelse(u_grad_diff > 0, "Increase", "Decrease")) %>% 
  mutate(u_grad_diff = abs(u_grad_diff)) %>% 
  group_by(positive)

sep_joined %>% 
  ggplot(aes(x = title, y = u_grad_diff)) +
    geom_col() +
    facet_wrap(~ positive) +
    labs(y = "Change", x = "Course Title") +
    coord_flip() +
    ggtitle("Classes with 5 Biggest Increases and 
            \n 5 Biggest Decreases in Enrollment") +
  theme_classic()
